...
def create_3d_data():
    ...
    array = []
    for grasp in grasps:
        hand_des = HandDescriptor(HandDescriptor.T_from_approach_axis_center(grasp.approach, grasp.axis,
                                                                             (grasp.bottom+grasp.top)/2),
                                  32)
        hand_des.generate_3d_binary_image(cloud)
        array.append(hand_des.image)
    array = np.stack(array, 0)
    ...
    try:
        ...
        raw_input('write?')
        np.save(f, array)
        print 'current data: ' + str(array.shape[0])
