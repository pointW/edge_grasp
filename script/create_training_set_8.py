...
p = [-0.09, 1.0, 0.1]
grasp_space = [(p[0]-0.11, p[0]+0.15), (p[1]-0.07, p[1]+0.07), (p[2]-0.12, p[2]+0.07)]
...
cloud = point_cloud.LoadPcd(os.path.dirname(__file__) + '/../data/pcd/cloud7.pcd')
cloud = point_cloud_util.filter_workspace(workspace, cloud)
# T = np.eye(4)
# T[0:3, 3] = [0, -0.3, 0]
# cloud = point_cloud_util.transform(T, cloud)
...
def create_data():
    ...
    f_3d = os.path.dirname(__file__) + '/../data/normal_shelf_3d.npy'
    f_2d = os.path.dirname(__file__) + '/../data/normal_shelf_2d.npy'
    try:
        old_3d = np.load(f_3d)
        if old_3d.shape[0] != 0:
